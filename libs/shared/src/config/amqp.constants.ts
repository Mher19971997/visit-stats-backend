import { Assert, Bind } from '@visit_stats_backend/shared/src/amqp/amqp.entity';
...
export namespace amqpOptions {
  ...
  export const queue: { assert: Assert['queue'][]; bind: Bind['queue'][] } = {
    assert: [
      { queue: constants.QUEUE_CREATE_COUNTRY, options: { autoDelete: false, durable: true } },
    ],
    bind: [
      { queue: constants.QUEUE_CREATE_COUNTRY, source: constants.EXCHANGE_WORKER, pattern: `/send/${constants.CREATE_COUNTRY}` },
    ],
  };
}