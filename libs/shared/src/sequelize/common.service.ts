...
import { CommonModel } from '@visit_stats_backend/shared/src/sequelize/common-model';
import {
...
} from '@visit_stats_backend/shared/src/sequelize/common-entity';
import { FilterInput, IncludeOptions, ListResult, ProcessingResult, UUID } from '@visit_stats_backend/shared/src/sequelize/meta';
...
// import { cache } from '@visit_stats_backend/shared/src/decorator/cache.decorator';
import { l10n } from '@visit_stats_backend/shared/src/config/l10n-constants';
...
> {
  ...
  // @cache.Purge()
  async create(inputDto: C,  txn?: s.Transaction): Promise<E | null> {
    const transaction = <s.Transaction>(txn );
  ...
  async findAll(filter: F, txn?: s.Transaction): Promise<ListResult<E>> {
    const transaction = <s.Transaction>(txn);
  ...
  async findOne(filter: F, txn?: s.Transaction): Promise<E | null> {
    const transaction = <s.Transaction>(txn);
  ...
  // @cache.Purge()
  async update(filter: F, inputDto: U, txn?: s.Transaction): Promise<E | null> {
    const transaction = <s.Transaction>(txn);
  ...
  // @cache.Purge()
  async upsert(filter: F, inputDto: C | U, txn?: s.Transaction): Promise<E | null> {
    const transaction = <s.Transaction>(txn);
  ...
  // @cache.Purge()
  async remove(filter: F, txn?: s.Transaction): Promise<E | null> {
    const transaction = <s.Transaction>(txn);
    const res = await this.findOne(filter, transaction);
    if (!res) {
      return null;
